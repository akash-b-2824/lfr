            ...
            for job in pending:
                ...
                if path1:
                    ...
                    if path2:
                        ...
                        # Join instructions. Note: path2[0] is pickup node, already reached at end of path1
                        # path_to_instr_list returns len(path)-1 instructions.
                        full_instr = instr1 + (instr2[1:] if len(instr2) > 0 else [])
                        
                        # Create execution plan: [[node, cmd], [node, cmd] ... [end, 'D']]
                        plan = []
                        ...
                        if len(full_path) - 1 == len(full_instr):
                            ...
                            # Fallback (wait) if mismatch usually due to wait step
                            plan.append([full_path[-1], 'D'])

                        job['assigned_robot'] = rid
                        job['status'] = 'assigned'
                        job['path'] = full_path
                        job['plan'] = plan # <--- SENDING PLAN
...
def request_path():
    # (This endpoint was for manual requests, but we keep it consistent)
...
def report_execution():
    ...
    with state_lock:
        ...
        if jid and jid in jobs:
            jobs[jid].setdefault('reports', []).append({'robot': rid, 'ts': time.time()})
            jobs[jid]['status'] = 'done'
...
function updateUI() {
    ...
    Object.keys(ROBOTS).forEach(id => {
        const r = ROBOTS[id];
        rList.innerHTML += `<div class="list-item"><span style="color:${r.color}">●</span> <strong>${id.substring(0,4)}</strong> @ ${r.node} (${r.status})</div>`;
    });
    const jList = document.getElementById('jobs-list');
    jList.innerHTML = '';
    Object.values(JOBS).sort((a,b)=>b.submitted_ts-a.submitted_ts).forEach(j => {
        jList.innerHTML += `<div class="list-item"><strong>#${j.id.substring(0,4)}</strong> ${j.pickup}->${j.drop} [${j.status}]</div>`;
    });
    drawMap();
}
...
function drawMap() {
    ...
    for(let n in NODE_COORDS) {
        ...
        c.setAttribute('cx', p.x); c.setAttribute('cy', p.y); c.setAttribute('r', 4); c.setAttribute('fill', '#333');
    ...
    for(let id in ROBOTS) {
        ...
        const c = document.createElementNS('http://www.w3.org/2000/svg','circle');
        c.setAttribute('cx', p.x); c.setAttribute('cy', p.y); c.setAttribute('r', 8); c.setAttribute('fill', r.color);
        viewport.appendChild(c);
    }
}
function submitJob() {
