...
# Line-following state flags
rec = True              # allow node recognition
lef = False             # expecting left-based turn
rig = False             # expecting right-based turn
k = 0                   # current plan step index
b = -1                  # last reported step index (prevents repeated updates)
job_id = None           # active job id from server
plan = []               # current job plan list from server

# Robot identity
ROBOT_ID_FINAL = None   # resolved MAC-based robot ID

# Path trace buffer for reporting to server
_nodes_with_dir_trace = []

# Current direction cached from server start_dir
curr = "s"
...
def mainline():
    ...
    while True:
        def preo():
            ...
            global lef,k,b,job_id
            if b!=k:
                send_location_update(plan[k][0], direction, k)
                b=k
            if plan[k][1]=='L':
                lef=True
                rig=False
                change_dir("r")
            elif plan[k][1]=='D' or plan[k][1]=='B':
                stop()
                sleep(3)
                blink(5, 0.02)
                if plan[k][1]=='D':
        ...
        while True:
            ...
            if (bl==1 or br==1)and(rec and (not rig)and(not lef)):
                ...
                sleep(see)
                if fl==0 and fs==1 and fr==0:
                    forward()
                    rec=True
            elif fl==1 and fs==1 and fr==0:
                sleep(see)
                if fl==1 and fs==1 and fr==0:
                    tilt_left()
                    rec=True
            elif fl==1 and fs==0 and fr==0:
                sleep(see)
                if fl==1 and fs==0 and fr==0:
                    turn_left()
            elif fl==0 and fs==1 and fr==1:
                sleep(see)
                if fl==0 and fs==1 and fr==1:
                    tilt_right()
                    rec=True
            elif fl==0 and fs==0 and fr==1:
                sleep(see)
                if fl==0 and fs==0 and fr==1:
                    turn_right()
            elif fl==1 and fs==1 and fr==1:
                sleep(see)
                if fl==1 and fs==1 and fr==1:
                    forward()
            sleep(0.001)   
# ---------- CONFIG ----------
WIFI_SSID = "A"
WIFI_PASS = "77777777"
SERVER_BASE = "http://192.168.155.98:8080/"  # change to your server ip/port
