...
# Safe zones where robots can idle indefinitely
PARKING_NODES = ['11', '12', '13', '15', '26', '31', '46', '51', '56', '81', '82', '83', '84', '85', '86']
...
def is_safe(node, t, my_id, prev_node=None):
    ...
    if res_id and res_id != my_id: return False
...
def space_time_a_star(graph, start, end, start_time, my_id, max_time=MAX_SEARCH_DEPTH):
    ...
    while open_set:
        ...
        neighbors.append(current) # Wait action
        ...
        for neighbor in neighbors:
            ...
            if is_safe(neighbor, next_time, my_id, current):
                ...
                if neighbor == current: new_g += 1.1 # Wait penalty
                
                new_f = new_g + h
                heapq.heappush(open_set, (new_f, new_g, neighbor, path + [neighbor]))
    return None

def find_nearest_parking(current_node):
    """Finds closest PARKING_NODE that is not occupied by an IDLE robot."""
    candidates = []
    for p in PARKING_NODES:
        # Check if occupied
        is_occupied = False
        for r in robots.values():
            if r['node'] == p: is_occupied = True; break
        
        if not is_occupied:
            dist = get_manhattan_dist(current_node, p)
...
def create_system_job(pickup, drop, robot_id):
    """Creates a job specifically for moving a robot (Parking/Nudge)"""
...
def update_location():
    ...
    with state_lock:
        ...
        if node in path: robots[rid]['current_path'] = path[path.index(node):]
            
        if status == 'job_done':
            # 1. Mark current job done
            jid = robots[rid].get('current_job')
            ...
            # 3. AUTO-PARKING LOGIC
            # If we finished a job and are NOT in a parking node, move immediately
            if node not in PARKING_NODES:
                parking_spot = find_nearest_parking(node)
                if parking_spot:
                    # Generate a parking job
                    ...
                    if park_path:
                        ...
                        # Path failed (blocked?), try again later or stay put
...
def reset_sim():
    with state_lock:
        ...
        for r in robots.values():
            ...
            socketio.emit('robot_update', {'robot': r['id'] if 'id' in r else 'unknown', 'info': r})
    return jsonify({'ok':True}), 200
