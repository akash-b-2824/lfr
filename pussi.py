...
<body style="text-align: center; font-family: Arial, sans-serif; background-color: #f2f2f2; padding: 20px;">
    <h2 style="color: navy;">Enter Locations</h2>
    <form action="/" method="GET" style="background: white; padding: 15px; border-radius: 8px; box-shadow: 2px 2px 10px gray; display: inline-block;">
        <label for="location">Call Location:</label>
        <input type="text" id="location" name="final" style="padding: 5px; margin: 5px;"><br>
        <input type="submit" value="Submit" style="background-color: navy; color: white; padding: 5px 10px; border: none; cursor: pointer; border-radius: 5px;">
    </form>
</body>
</html>
...
end_node=""
def mainn(gra):      
    f = con()  # Assuming con() is defined elsewhere
    global initial_direction, start_node, end_node,traversal_nodes
    ...
    def navigate_robot(start, end, initial_direction, graph):
        ...
        traversal_nodes.append(path[-1])  # Append the final node
        return path, traversal_nodes, commands, current_direction
    ...
    path, traversal_nodes, commands, initial_direction = navigate_robot(start_node, end_node, initial_direction, graph)
    start_node = end_node
    return commands,traversal_nodes
